...
런타임시에 항상 필요한 jar 파일의 위치를 정해줄거라면 컴퓨터의 `CLASSPATH` 환경변수를 지정해주는 것이 좋다. 그렇게 하면 런타임동안 어플리케이션 클래스 로더가 그 변수 내의 지정된 경로에서 jar 파일과 클래스를 스캔할 것이다.
...
### DOS Prompt에 임시로 classpath 설정하기

cmd창에서 클래스패스를 임시로 설정해주고 싶다면 `set CLASSPATH` 명령어를 사용하면 된다. 디렉토리나 아카이브(`.zip` 또는 `.jar` 파일), 또는 `*`가 아닌 classpath 항목은 무시된다.
