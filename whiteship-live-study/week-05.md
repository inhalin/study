...
여러가지 타입의 데이터를 그리는 메소드가 들어있는 클래스가 있다고 가정해보자. 각 메소드의 매개변수 타입이 다르다면 메소드마다 drawInteger(int a), drawString(String str), drawFloat(float f) 등 메소드 이름을 다르게 주지 않고도 draw(int a), draw(String str), draw(float f) 등과 같이 매개변수의 타입이 다른 draw라는 같은 이름의 메소드를 중복으로 가질 수 있다는 의미이다.
...
예를 들어 매개변수로 실수 두개를 받아 나누는 메소드를 구현할 때, 0으로 나누면 에러가 발생하기 때문에 계산 전에  `divisor`의 값이 0인지 확인한다. 만약 `divisor`의 값이 0이라면 리턴문을 이용해 메소드에서 빠져나온다.
